...
class Brain:
    ...
    def run_rnn(self):
        ...
        for i in range(self.iterations + 1):
            if self.batch_size is not None:
                mask = np.random.choice(self.data.X_train.size(0), self.batch_size, replace=False)
                X_train, y_train = self.data.load_train(mask)
                y_predict = self.net(X_train)
                loss = self.__criterion(y_predict, y_train)
            else:
                ...
                with torch.no_grad():
                    if self.batch_size is not None:
                        ...
                        for j in range(num_batch):
                            mask = j * self.batch_size + np.arange(self.batch_size)
                            X_test, y_test = self.data.load_test(mask)
                            y_test_pred = self.net(X_test)
                            loss_test += self.__criterion(y_test_pred, y_test).detach()
